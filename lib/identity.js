...
function mergeEvents(events = []) {
  ...
  for (const { fingerprint, records } of groups) {
    ...
    for (const r of records) {
      ...
      const candImg = r.canonical_image_url || r.image;
      if (candImg) {
        const primImg = primary.canonical_image_url || primary.image;
        if (!primImg) {
          primary.canonical_image_url = candImg;
          primary.image = candImg;
        } else if (typeof primImg === 'string' && primImg.includes('unsplash.com') && !candImg.includes('unsplash.com')) {
          primary.canonical_image_url = candImg;
          primary.image = candImg;
        }
      }
