...
class CommunityPostDurableStore {
  ...
  _serializeEnvelope(post = {}) {
    ...
    const meta = {
      ...
      adminApprovedLevel: Number(post.adminApprovedLevel || 1),
      adminDenied: Boolean(post.adminDenied),
      adminDenialReason: post.adminDenialReason || null,
      adminDeniedLevels: Array.isArray(post.adminDeniedLevels) ? post.adminDeniedLevels : [],
  ...
  _deserializeRow(row = {}) {
    ...
    return {
      ...
      adminApprovedLevel: Number(meta.adminApprovedLevel || 1),
      adminDenied: Boolean(meta.adminDenied),
      adminDenialReason: meta.adminDenialReason || null,
      adminDeniedLevels: Array.isArray(meta.adminDeniedLevels) ? meta.adminDeniedLevels : [],
  ...
  getAllPosts() {
    ...
    const seen = new Set();
    const list = [];
    for (const post of this.memoryPosts.values()) {
      if (!post || !post.id || seen.has(post.id)) continue;
      seen.add(post.id);
      if (this.deletedPostIds.has(post.id)) continue;
      const postUuid = extractUuidFromId(post.id);
      if (postUuid && this.deletedPostIds.has(postUuid)) continue;
      list.push(post);
    }
    return list;
  }
