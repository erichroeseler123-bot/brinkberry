...
class CommunityPostDurableStore {
  ...
  async syncFromDurableStore(force = false) {
    ...
    try {
      ...
      if (resActive.ok) {
        ...
        console.error('[CommunityStore] Supabase syncActive error:', resActive.status, await resActive.text());
      }
  ...
  async persistPost(post = {}) {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('[CommunityStore] Supabase persist error:', res.status, await res.text());
      }

      return res.ok;
    } catch (err) {
      console.error('[CommunityStore] Supabase persist exception:', err);
      return false;
    }
  }
  ...
  async fetchPostByIdRemote(id) {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('[CommunityStore] Supabase fetchById error:', res.status, await res.text());
        return null;
      }
      const rows = await res.json();
      if (!Array.isArray(rows) || rows.length === 0) {
        console.warn('[CommunityStore] Supabase fetchById empty rows for:', postUuid);
        return null;
      }
      ...
      console.error('[CommunityStore] Supabase fetchById exception:', err);
      return null;
    }
  }
