...
class CommunityPostDurableStore {
  ...
  get authKey() {
    return (this.serviceKey && !this.serviceKey.includes('SENSITIVE')) ? this.serviceKey : this.publishableKey;
  }
  ...
  async syncFromDurableStore(force = false) {
    ...
    // 1. Sync from GCS if configured (production multi-instance storage)
    ...
    // 2. Fallback: sync from Supabase canonical_events with publishable key or service key
    try {
  ...
  async persistPost(post = {}) {
    ...
    // 2. Persist to GCS bucket if configured
    if (this.isGcsConfigured) {
      try {
    ...
    // 3. Dual-persist to Supabase canonical_events if service role key is valid
    if (this.supabaseUrl && this.serviceKey && !this.serviceKey.includes('SENSITIVE') && this.serviceKey.length > 20) {
      try {
  ...
  async updatePost(post = {}) {
    ...
    if (postUuid && this.supabaseUrl && this.serviceKey && !this.serviceKey.includes('SENSITIVE') && this.serviceKey.length > 20) {
      try {
        const { envelopeDesc } = this._serializeEnvelope(post);
        await fetch(`${this.supabaseUrl}/rest/v1/canonical_events?id=eq.${encodeURIComponent(postUuid)}`, {
          method: 'PATCH',
          headers: {
            apikey: this.authKey,
            authorization: `Bearer ${this.authKey}`,
  ...
  async deletePost(id, deletionReason = 'user_deleted') {
    ...
    // 1. Delete and record tombstone in GCS
    if (this.isGcsConfigured) {
      try {
    ...
    // 2. Patch Supabase if service key is valid
    if (postUuid && this.supabaseUrl && this.serviceKey && !this.serviceKey.includes('SENSITIVE') && this.serviceKey.length > 20) {
      try {
  ...
  async fetchPostByIdRemote(id) {
    ...
    // 1. Fetch from GCS if configured
    if (this.isGcsConfigured) {
      try {
    ...
    // 2. Fetch from Supabase canonical_events with publishable key or service key
    if (postUuid) {
      try {
