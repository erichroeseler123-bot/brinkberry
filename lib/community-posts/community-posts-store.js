...
class CommunityPostDurableStore {
  ...
  _deserializeRow(row = {}) {
    ...
    const isDeleted = Boolean(row.active === false || meta.isDeleted);
    const isDisplayable = Boolean(row.active !== false && meta.isDisplayable !== false && !isDeleted);

    return {
      id: meta.cleanId || displayId,
      postUuid,
      fingerprint: meta.auditRecord?.fingerprint || crypto.createHash('sha256').update(`${row.title}:${row.start_time}:${row.lat}:${row.lon}`).digest('hex'),
      title: row.title || 'Community Event',
      description: cleanDesc,
      category: row.category || 'community',
      category_tags: meta.category_tags || [row.category || 'community'],
      start_time: row.start_time,
      start: row.start_time,
      end_time: row.end_time || row.start_time,
      end: row.end_time || row.start_time,
      venue: row.venue || 'Community Location',
      venue_name: row.venue || 'Community Location',
      address: row.address || null,
      city: row.city || 'Nearby',
      lat: row.lat != null ? Number(row.lat) : null,
      lon: row.lon != null ? Number(row.lon) : null,
      venue_latitude: row.lat != null ? Number(row.lat) : null,
      venue_longitude: row.lon != null ? Number(row.lon) : null,
      ...
      detailsUrl: row.ticket_url || meta.detailsUrl || null,
      contact: meta.contact || null,
      source: row.source || 'community_post',
      ...
      price_min: 0,
      price_max: 0,
      priceDisplay: row.price_display || 'Free / Walk-in',
      ...
      isDisplayable,
      isDeleted,
      deletionKey: meta.deletionKey || row.manage_token || null,
      deletionKeyHash: meta.deletionKeyHash || null,
      auditRecord: meta.auditRecord || null,
      email_provided: meta.email_provided || row.submitter_email || null,
  ...
  async syncFromDurableStore(force = false) {
    ...
    try {
      // 1. Fetch active community events
      const resActive = await fetch(`${this.supabaseUrl}/rest/v1/community_events?active=eq.true&order=start_time.asc`, {
        headers: {
          apikey: this.authKey,
      ...
      // 2. Fetch deleted / inactive community event IDs to sync tombstones across instances
      const resDeleted = await fetch(`${this.supabaseUrl}/rest/v1/community_events?active=eq.false&select=id`, {
        headers: {
          apikey: this.authKey,
          authorization: `Bearer ${this.authKey}`
        },
  ...
  async persistPost(post = {}) {
    ...
    try {
      ...
      const manageToken = (post.deletionKey && extractUuidFromId(post.deletionKey)) ? post.deletionKey : crypto.randomUUID();
      ...
      const payload = {
        ...
        active: !post.isDeleted && post.isDisplayable,
        submitter_email: post.email_provided || post.verifiedEmail || null,
  ...
  async updatePost(post = {}) {
    ...
    try {
      const { envelopeDesc } = this._serializeEnvelope(post);
      const res = await fetch(`${this.supabaseUrl}/rest/v1/community_events?id=eq.${encodeURIComponent(postUuid)}`, {
        method: 'PATCH',
        headers: {
          apikey: this.authKey,
          authorization: `Bearer ${this.authKey}`,
