  ...
  const ladder = BROADCAST_LEVELS.map(lvl => {
    ...
    if (lvl.level <= targetLevelNum) {
      if (lvl.level === 1) {
        ...
        if (highestApprovedLevel === 1 && isEmailConfirmed) {
          status = 'approved';
          ...
          status = 'pending';
          statusReason = 'Email provided — enter confirmation code to unlock Hood reach';
        } else {
          status = 'pending';
        ...
        if (hasEmail && isPhoneConfirmed) {
          ...
          status = 'pending';
          statusReason = 'Phone provided — enter SMS verification code to unlock Quadrant reach';
        } else {
          status = 'pending';
...
async function fetchCommunityPostById(id) {
  const local = getCommunityPostById(id);
