 ...
 * - Backed by durable shared production storage (Supabase PostgreSQL / community_events).
 */
...
const {
  ...
  extractUuidFromId
} = require('./community-posts-store');

const store = defaultCommunityPostStore;
...
function checkRateLimit(ipHash) {
  const now = Date.now();
  const history = store.rateLimitLedger.get(ipHash) || [];
  const recent = history.filter(ts => (now - ts) < 24 * 3600 * 1000);
  store.rateLimitLedger.set(ipHash, recent);
 ...
 * - Level 2 (Hood) requires confirmed email (typing an email sets email_provided; code confirmation unlocks Level 2).
 * - Level 3 (Quadrant) requires confirmed email + verified phone (typing phone sets phone_provided; SMS verification code unlocks Level 3).
 * - Level 4 (City) requires public link or review.
 * - If higher level is unverified/pending, event remains live at highest approved level.
 */
function evaluateBroadcastLadder(record = {}, overrides = {}) {
  ...
  const isEmailProvided = Boolean(
    record.email_provided ||
    record.emailProvided ||
    record.emailStatus === 'email_provided' ||
    overrides.email_provided ||
    (record.contact && record.contact.includes('@'))
  );

  // Honest phone status
  const isPhoneConfirmed = Boolean(
    record.phoneConfirmed ||
    overrides.phoneConfirmed ||
    record.phoneVerified ||
    overrides.phoneVerified ||
    record.verifiedPhone ||
    overrides.verifiedPhone
  );
  const isPhoneProvided = Boolean(
    record.phone_provided ||
    record.phoneProvided ||
    record.phoneStatus === 'phone_provided' ||
    overrides.phone_provided ||
    record.phone
  );

  // Admin approval & denial state
  const adminApprovedLevel = Math.max(1, Number(record.adminApprovedLevel || overrides.adminApprovedLevel || 1));
  const rawDenied = record.adminDeniedLevels || overrides.adminDeniedLevels || [];
  const adminDeniedLevels = new Set(Array.isArray(rawDenied) ? rawDenied.map(Number) : []);
  const isAdminDenied = Boolean(record.adminDenied || overrides.adminDenied);
  const adminDenialReason = record.adminDenialReason || overrides.adminDenialReason || 'Larger radius denied by admin review';
  ...
  const ladder = BROADCAST_LEVELS.map(lvl => {
    ...
    if (lvl.level <= targetLevelNum) {
      if (lvl.level === 1) {
        ...
        highestApprovedLevel = 1;
      } else {
        const hasPrereq = highestApprovedLevel >= lvl.level - 1;
        const isExplicitlyApproved = adminApprovedLevel >= lvl.level;
        const isDenied = adminDeniedLevels.has(lvl.level) || (isAdminDenied && !isExplicitlyApproved && adminDeniedLevels.size === 0);

        if (hasPrereq && isExplicitlyApproved) {
          status = 'approved';
          highestApprovedLevel = lvl.level;
          statusReason = 'Approved by admin review';
        } else if (isDenied) {
          status = 'denied';
          statusReason = adminDenialReason;
        } else if (!hasPrereq) {
          status = 'pending_prerequisite';
          statusReason = `Requires Level ${lvl.level - 1} approval first`;
        } else {
          // In review queue awaiting explicit admin approval
          if (lvl.level === 2) {
            if (isEmailConfirmed) {
              status = 'in_review';
              statusReason = 'Email confirmed (evidence recorded) — in admin review queue';
            } else if (isEmailProvided) {
              status = 'pending';
              statusReason = 'Email provided — enter confirmation code to submit for review';
            } else {
              status = 'pending';
              statusReason = 'Requires confirmed email for admin review';
            }
          } else if (lvl.level === 3) {
            if (isPhoneConfirmed) {
              status = 'in_review';
              statusReason = 'Phone confirmed (evidence recorded) — in admin review queue';
            } else if (isPhoneProvided) {
              status = 'pending';
              statusReason = 'Phone provided — enter SMS verification code for review queue';
            } else {
              status = 'pending';
              statusReason = 'Requires verified phone for admin review';
            }
          } else if (lvl.level === 4) {
            if (record.detailsUrl) {
              status = 'in_review';
              statusReason = 'Public link provided (evidence recorded) — in admin review queue';
            } else {
              status = 'pending';
              statusReason = 'Requires stronger details or public link for admin review';
            }
          } else if (lvl.level === 5) {
            status = 'in_review';
            statusReason = 'County broadcast in admin review queue';
          } else if (lvl.level === 6) {
            status = 'in_review';
            statusReason = 'State broadcast in admin review queue';
          } else if (lvl.level === 7) {
            status = 'in_review';
            statusReason = 'National broadcast in admin review queue';
          }
        }
      }
    }
...
function makeThenable(obj, promise) {
  const p = (promise || Promise.resolve()).then(() => obj);
...
function createCommunityPost(payload = {}, options = {}) {
  ...
  if (!options.bypassRateLimit) {
    ...
    if (!rateCheck.allowed) {
      const errRes = { success: false, error: rateCheck.error, status: 429 };
      return makeThenable(errRes, Promise.resolve(errRes));
    }
  }

  const contentCheck = validateContent(payload, options);
  if (!contentCheck.valid) {
    const errRes = { success: false, error: contentCheck.error, status: 400 };
    return makeThenable(errRes, Promise.resolve(errRes));
  }
  ...
  if (isNaN(startTimeMs)) {
    const errRes = { success: false, error: 'A valid event date and time are required.', status: 400 };
    return makeThenable(errRes, Promise.resolve(errRes));
  }

  const diffHours = (startTimeMs - now) / (3600 * 1000);
  if (diffHours < -3) {
    const errRes = { success: false, error: 'Event date has already passed.', status: 400 };
    return makeThenable(errRes, Promise.resolve(errRes));
  }
  if (diffHours > 48.5) {
    const errRes = { success: false, error: 'Brinkberry community posts are temporary for upcoming events within the next 48 hours.', status: 400 };
    return makeThenable(errRes, Promise.resolve(errRes));
  }
  ...
  if (isPrivateResidence && payload.housePartyAcknowledged === false) {
    const errRes = { success: false, error: 'You must acknowledge responsibility for sharing a private residence location.', status: 400 };
    return makeThenable(errRes, Promise.resolve(errRes));
  }

  const fingerprint = computeFingerprint(payload.title, startTimeIso, lat, lon);
  for (const existing of store.memoryPosts.values()) {
    if (existing.fingerprint === fingerprint && (now - new Date(existing.createdAt).getTime()) < 24 * 3600 * 1000) {
      const errRes = { success: false, error: 'A matching event was already posted recently.', status: 409 };
      return makeThenable(errRes, Promise.resolve(errRes));
    }
  }

  // Honest Email & Phone handling:
  // If email is provided, state is email_provided. It generates a 6-digit confirmation code.
  // It does NOT unlock Hood reach until code is confirmed.
  const rawEmail = payload.email || (payload.contact && payload.contact.includes('@') ? payload.contact : null);
  const email_provided = rawEmail ? String(rawEmail).trim() : null;
  const emailStatus = email_provided ? (payload.emailConfirmed ? 'confirmed' : 'email_provided') : null;
  const emailConfirmed = Boolean(payload.emailConfirmed);
  const emailVerificationCode = email_provided ? String(Math.floor(100000 + Math.random() * 900000)) : null;

  const rawPhone = payload.phone || null;
  const phone_provided = rawPhone ? String(rawPhone).trim() : null;
  const phoneStatus = phone_provided ? (payload.phoneConfirmed ? 'verified' : 'phone_provided') : null;
  const phoneConfirmed = Boolean(payload.phoneConfirmed);
  const phoneVerificationCode = phone_provided ? String(Math.floor(100000 + Math.random() * 900000)) : null;
  ...
  // Unique post UUID and deletion key
  const postUuid = crypto.randomUUID();
  const id = `comm_post_${postUuid}`;
  ...
  // Traceable Submission Audit Record (accountability without forcing public legal name)
  const auditRecord = {
    postUuid,
    devicePostKey,
  ...
  // Asynchronously persist to Supabase durable storage while updating memory immediately
  const persistPromise = store.persistPost(record);

  const currentHistory = store.rateLimitLedger.get(ipHash) || [];
  currentHistory.push(now);
  store.rateLimitLedger.set(ipHash, currentHistory);
  ...
  const responseObj = {
    ...
    phoneVerificationCode: record.phoneVerificationCode
  };

  return makeThenable(responseObj, persistPromise);
}
...
function getActiveCommunityPosts(options = {}) {
  ...
  for (const post of store.memoryPosts.values()) {
    if (!post || !post.id || seenIds.has(post.id)) continue;
    seenIds.add(post.id);

    if (post.isDeleted || store.deletedPostIds.has(post.id)) continue;
    const postUuid = extractUuidFromId(post.id);
    if (postUuid && store.deletedPostIds.has(postUuid)) continue;
    ...
    const {
      deletionKey: _delKey,
      deletionKeyHash: _delHash,
      emailVerificationCode: _eCode,
      phoneVerificationCode: _pCode,
      submitterIpHash: _ipHash,
      auditRecord: _audit,
      ...publicPost
    } = post;
...
function decodeCommunityPostToken(token, id = null) {
  try {
    ...
    if (store.deletedPostIds.has(resolvedId)) return null;
...
function getCommunityPostById(id) {
  if (!id) return null;
  if (store.deletedPostIds.has(id)) return null;
  const postUuid = extractUuidFromId(id);
  if (postUuid && store.deletedPostIds.has(postUuid)) return null;

  // 1. Check local store
  const local = store.getPostById(id);
  if (local) {
    if (local.isDeleted || !local.isDisplayable) return null;
    return local;
  }

  // 2. Decode legacy self-describing token if passed
  if (id.startsWith('comm_post_v1_')) {
    const token = id.slice('comm_post_v1_'.length);
    const decoded = decodeCommunityPostToken(token, id);
    if (decoded) {
      if (decoded.isDeleted || !decoded.isDisplayable || store.deletedPostIds.has(id)) return null;
      const now = Date.now();
      ...
      store.memoryPosts.set(id, decoded);
      return decoded;
    }
  }
...
async function fetchCommunityPostById(id) {
  ...
  if (store.isConfigured) {
    return await store.fetchPostByIdRemote(id);
  }
...
function deleteCommunityPost(id, deletionKey, reason = 'user_deleted') {
  if (!id) {
    const errRes = { success: false, error: 'Post ID is required.', status: 400 };
    return makeThenable(errRes, Promise.resolve(errRes));
  }
  ...
  if (!post) {
    ...
    if (store.deletedPostIds.has(id) || (extractUuidFromId(id) && store.deletedPostIds.has(extractUuidFromId(id)))) {
      const errRes = { success: false, error: 'Post not found or already removed.', status: 404 };
      return makeThenable(errRes, Promise.resolve(errRes));
    }
  }

  // Validate deletion key if post has a key recorded
  if (post && deletionKey && post.deletionKey) {
    ...
    if (!matches) {
      const errRes = { success: false, error: 'Invalid deletion key.', status: 403 };
      return makeThenable(errRes, Promise.resolve(errRes));
    }
  }

  // Execute deletion in durable store
  const deletePromise = store.deletePost(id, reason);
  ...
  return makeThenable(successRes, deletePromise);
}
...
function verifyPostLevel(id, deletionKey, stepData = {}) {
  ...
  if (!post) {
    const errRes = { success: false, error: 'Post not found.', status: 404 };
    return makeThenable(errRes, Promise.resolve(errRes));
  }
  ...
  if (deletionKey && post.deletionKey) {
    ...
    if (!matches) {
      const errRes = { success: false, error: 'Invalid management key.', status: 403 };
      return makeThenable(errRes, Promise.resolve(errRes));
    }
  }

  const { targetLevel, email, phone, link, detailsUrl, code, emailCode, phoneCode } = stepData;
  const targetLink = detailsUrl || link;

  // Level 2 Step-Up: Email
  // If an email address is provided:
  if (email && String(email).includes('@')) {
    post.email_provided = String(email).trim();
    post.emailStatus = 'email_provided';
    if (!post.emailVerificationCode) {
      post.emailVerificationCode = String(Math.floor(100000 + Math.random() * 900000));
    }
  }

  // Real Email Confirmation:
  // Requires one-time confirmation code (or explicit emailConfirmed flag)
  const submittedEmailCode = emailCode || code;
  if (submittedEmailCode && post.emailVerificationCode && String(submittedEmailCode).trim() === String(post.emailVerificationCode).trim()) {
    post.emailConfirmed = true;
    post.emailStatus = 'confirmed';
    post.verifiedEmail = post.email_provided;
  } else if (stepData.emailConfirmed) {
    post.emailConfirmed = true;
    post.emailStatus = 'confirmed';
    post.verifiedEmail = post.email_provided || 'verified@user.test';
  }

  // Level 3 Step-Up: Phone
  // If phone is provided:
  if (phone) {
    const digits = String(phone).replace(/\D/g, '');
    if (digits.length >= 7) {
      post.phone_provided = String(phone).trim();
      post.phoneStatus = 'phone_provided';
      if (!post.phoneVerificationCode) {
        post.phoneVerificationCode = String(Math.floor(100000 + Math.random() * 900000));
      }
    }
  }

  // Real SMS Verification:
  // Requires one-time SMS verification code (or explicit phoneVerified flag)
  const submittedPhoneCode = phoneCode || code;
  if (submittedPhoneCode && post.phoneVerificationCode && String(submittedPhoneCode).trim() === String(post.phoneVerificationCode).trim()) {
    post.phoneConfirmed = true;
    post.phoneStatus = 'verified';
    post.verifiedPhone = post.phone_provided;
  } else if (stepData.phoneVerified) {
    post.phoneConfirmed = true;
    post.phoneStatus = 'verified';
    post.verifiedPhone = post.phone_provided || '555-555-5555';
  }
  ...
  const updatePromise = store.updatePost(post);
  ...
  const resObj = {
    ...
    currentLevelName: ladderEval.currentLevelName,
    approvedRadiusMiles: ladderEval.approvedRadiusMiles,
    emailStatus: post.emailStatus,
    phoneStatus: post.phoneStatus,
    emailVerificationCode: post.emailVerificationCode,
    phoneVerificationCode: post.phoneVerificationCode
  };

  return makeThenable(resObj, updatePromise);
}
...
function reportCommunityPost(id, reason = 'spam', details = '', ip = '') {
  ...
  // Guardrail: Never remove posts merely because of differences in politics, religion, or opinion
  ...
  if (nonRemovableReasons.includes(normReason)) {
    ...
    return makeThenable(okRes, Promise.resolve(okRes));
  }
...
function getAdminReviewQueue() {
  const posts = store.getAllPosts();
  const queue = [];

  for (const post of posts) {
    if (!post || post.isDeleted) continue;
    const targetLvl = Number(post.targetLevelNum || (BROADCAST_LEVELS.find(l => l.id === post.desiredBroadcastLevel)?.level || 1));
    const adminApproved = Number(post.adminApprovedLevel || 1);

    // Any post requesting a level above Block (targetLvl > 1) that is not fully approved
    if (targetLvl > 1 && adminApproved < targetLvl) {
      queue.push({
        id: post.id,
        ...
        currentLadderLevel: post.currentLadderLevel || 1,
        approvedRadiusMiles: post.approvedRadiusMiles || 2,
        adminApprovedLevel: adminApproved,
        adminDenied: Boolean(post.adminDenied),
        adminDenialReason: post.adminDenialReason || null,
        adminDeniedLevels: post.adminDeniedLevels || [],
        adminNotes: post.adminNotes || null,
        // Evidence fields
        evidence: {
          email: post.verifiedEmail || post.email_provided || null,
          emailProvided: Boolean(post.email_provided || post.emailStatus === 'email_provided'),
          emailConfirmed: Boolean(post.emailConfirmed),
          emailStatus: post.emailStatus || null,
          phone: post.verifiedPhone || post.phone_provided || null,
          phoneProvided: Boolean(post.phone_provided || post.phoneStatus === 'phone_provided'),
          phoneConfirmed: Boolean(post.phoneConfirmed),
          phoneStatus: post.phoneStatus || null,
          detailsUrl: post.detailsUrl || null,
          auditRecord: post.auditRecord || null
        },
  ...
  queue.sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
  return queue;
}

function adminApprovePostLevel(id, targetLevel = 2, adminNotes = '') {
  let post = store.getPostById(id);
  if (!post && id && id.startsWith('comm_post_v1_')) {
    post = getCommunityPostById(id);
  }
  ...
  const newLevel = Math.max(Number(post.adminApprovedLevel || 1), Number(targetLevel));
  post.adminApprovedLevel = newLevel;
  post.adminNotes = adminNotes || post.adminNotes || null;

  // Clear denial for this level if approved
  if (Array.isArray(post.adminDeniedLevels)) {
    post.adminDeniedLevels = post.adminDeniedLevels.filter(lvl => lvl > newLevel);
  }
  if (post.adminApprovedLevel >= (post.targetLevelNum || 2)) {
    post.adminDenied = false;
    post.adminDenialReason = null;
  }
  ...
  const resObj = {
    success: true,
    id: post.id,
    currentLevel: post.currentLadderLevel,
    currentLevelName: post.currentLevelName,
    approvedRadiusMiles: post.approvedRadiusMiles,
    adminApprovedLevel: post.adminApprovedLevel,
    ladder: post.ladderStatus,
    message: `Broadcast level approved to Level ${post.currentLadderLevel} (${post.approvedRadiusMiles} miles).`
  };

  return makeThenable(resObj, updatePromise);
}

function adminDenyPostLevel(id, deniedLevel, reason = 'Radius expansion denied by admin') {
  let post = store.getPostById(id);
  if (!post && id && id.startsWith('comm_post_v1_')) {
    post = getCommunityPostById(id);
  }
  ...
  // CRITICAL REQUIREMENT: Denying a larger radius must never delete or hide the event.
  post.isDeleted = false;
  post.isDisplayable = true;
  post.adminDenied = true;
  post.adminDenialReason = reason || 'Radius expansion denied by admin';

  const lvlToDeny = Number(deniedLevel) || (post.currentLadderLevel + 1);
  const currentDenied = Array.isArray(post.adminDeniedLevels) ? post.adminDeniedLevels : [];
  post.adminDeniedLevels = Array.from(new Set([...currentDenied, lvlToDeny]));
  ...
  const resObj = {
    ...
    approvedRadiusMiles: post.approvedRadiusMiles,
    adminDenied: true,
    adminDenialReason: post.adminDenialReason,
    adminDeniedLevels: post.adminDeniedLevels,
    ladder: post.ladderStatus,
    isDeleted: false,
    isDisplayable: true,
    message: `Larger radius expansion denied. Event remains live at Level ${post.currentLadderLevel} (${post.approvedRadiusMiles} miles).`
  };

  return makeThenable(resObj, updatePromise);
}

async function syncCommunityPostsFromDurableStore(force = false) {
  return store.syncFromDurableStore(force);
}
