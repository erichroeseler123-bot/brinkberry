  ...
  while ((match = scriptRegex.exec(htmlContent)) !== null) {
    ...
    try {
      ...
      for (const item of items) {
        ...
        const image = extractJsonLdImage(item.image) ||
                      extractJsonLdImage(item.photo) ||
                      extractJsonLdImage(item.thumbnailUrl) ||
                      null;
...
function extractJsonLdImage(val) {
  if (!val) return null;
  ...
  if (Array.isArray(val)) {
    for (const item of val) {
      const img = extractJsonLdImage(item);
      if (img) return img;
    }
    return null;
  }
  if (typeof val === 'object') {
    return extractJsonLdImage(val.url || val.contentUrl || val.thumbnailUrl || val.image);
  }
  return null;
}
