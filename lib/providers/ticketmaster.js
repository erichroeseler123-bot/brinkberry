 ...
 * Automatically marks provider inactive if credentials return 401 Unauthorized.
 */
...
// In-memory circuit breaker for invalid credentials
let ticketmasterAuthFailed = false;
...
async function fetchTicketmasterEvents(options = {}) {
  ...
  const apiKey = options.apiKey || process.env.TICKETMASTER_API_KEY || (fetchFn !== fetch ? 'test_tm_api_key' : null);
  const isConfigured = Boolean(apiKey && apiKey !== 'mock_tm_key' && !ticketmasterAuthFailed);

  // 1. If provider was marked inactive due to invalid credentials
  if (ticketmasterAuthFailed) {
    return {
      events: [],
      status: 'inactive',
      configured: false,
      count: 0,
      latencyMs: 0,
      reason: 'Ticketmaster credentials invalid (401); provider marked inactive'
    };
  }
  ...
  // 3. If API key is not configured
  ...
  const isNorthAmerica = lon >= -170 && lon <= -50 && lat >= 15 && lat <= 75;
  if (isNorthAmerica) {
    query.searchParams.set('radius', String(Math.round(radiusMiles)));
    query.searchParams.set('unit', 'miles');
  } else {
    query.searchParams.set('radius', String(Math.round(radiusMiles * 1.60934)));
    query.searchParams.set('unit', 'km');
  }
  ...
  try {
    ...
    if (/401|invalid.*apikey|unauthorized/i.test(err.message)) {
      ticketmasterAuthFailed = true;
      return {
        events: [],
        status: 'inactive',
        configured: false,
        count: 0,
        latencyMs: Date.now() - startTime,
        reason: 'Ticketmaster API key returned 401 Unauthorized; provider marked inactive'
      };
    }
...
function resetTicketmasterState() {
  ticketmasterAuthFailed = false;
}
