  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed === 'END:VEVENT') {
      if (currentEvent && currentEvent.summary && currentEvent.dtstart) {
        ...
        const isCivic = cat === 'civic' || feedMeta.defaultCategory === 'civic';
        const confirmationStatus = feedMeta.confirmationStatus || (isCivic ? 'official_government_calendar' : 'public_community_listing');
        ...
        events.push({
          ...
          category_tags: isCivic ? ['civic', 'politics'] : [cat],
          vibe_labels: isCivic ? ['civic', 'community'] : [cat, 'community'],
          price_status: 'free',
          price_min: 0,
          price_max: 0,
          price_display: 'Free',
          ...
          canonical_image_url: currentEvent.image || null,
          source: `community_${feedMeta.id}`,
          indoor_outdoor: 'unknown',
          hasTicket: false,
          sourceQualityLabel: isCivic ? 'Official government calendar' : 'Public community listing',
          confirmationStatus,
          lastVerifiedAt: nowIso,
          last_verified_at: nowIso,
          sourceEvidence: {
            sourceId: feedMeta.id,
            sourceUrl: currentEvent.url || feedMeta.provenance?.url || feedMeta.feedUrl,
            fetchedAt: nowIso,
    ...
    switch (propName) {
      ...
      case 'DESCRIPTION': {
        ...
        if (!currentEvent.image) {
          const imgMatch = propValue.match(/<img[^>]+src=["'](https?:\/\/[^"'>]+)["']/i);
          if (imgMatch) currentEvent.image = imgMatch[1];
        }
        break;
      }
      ...
      case 'IMAGE':
      case 'X-IMAGE':
      case 'X-LIVEWHALE-IMAGE': {
        const rawImg = propValue.replace(/\\,/g, ',').replace(/\\;/g, ';').trim();
        const urlMatch = rawImg.match(/https?:\/\/[^\s<>"'\\]+/i);
        if (urlMatch && !currentEvent.image) {
          let imgUrl = urlMatch[0];
          if (imgUrl.includes('/width/80/height/80/')) {
            imgUrl = imgUrl.replace('/width/80/height/80/', '/width/800/height/500/');
          }
          currentEvent.image = imgUrl;
        }
        break;
      }
    }
...
async function fetchCommunityFeedEvents(feedConfig, options = {}) {
  ...
  try {
    ...
    icsContent = typeof rawText === 'string' ? rawText : JSON.stringify(rawText);
  } catch (err) {
    const isMockFetch = Boolean(fetchFn && fetchFn !== fetch && (typeof globalThis !== 'undefined' && fetchFn !== globalThis.fetch));
    if ((options.useFallback || feedConfig.isStatic || (!isMockFetch && feedConfig.fallbackIcs)) && feedConfig.fallbackIcs) {
      icsContent = feedConfig.fallbackIcs;
    } else {
      return {
        feedId: feedConfig.id,
        name: feedConfig.name,
  ...
  // Time window boundary clamp
  const nowMs = Date.now();
  const maxAllowedMs = windowEnd ? new Date(windowEnd).getTime() : (nowMs + 48 * 3600e3);
  const minAllowedMs = windowStart ? new Date(windowStart).getTime() : (nowMs - 3600e3);
  const filtered = parsed.filter(e => {
    const t = new Date(e.start_time).getTime();
    return t >= minAllowedMs && t <= maxAllowedMs;
  });
