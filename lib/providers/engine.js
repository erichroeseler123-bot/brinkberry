...
async function executeHybridFeed(options = {}) {
  ...
  if (enableDynamic && Number.isFinite(lat) && Number.isFinite(lon)) {
    ...
    if (Array.isArray(tmResult.events)) {
      ...
      dynamicRawCount += tmResult.events.length;
    }
    if (Array.isArray(sgResult.events)) {
      providerResults.push(...sgResult.events);
      dynamicRawCount += sgResult.events.length;
    }
  }
  ...
  const anyDynamicConfigured = Object.values(providersStatus).some(p => p.configured);
  const anyDynamicActive = Object.values(providersStatus).some(p => p.status === 'ok' || p.status === 'cached' || p.status === 'mock');
