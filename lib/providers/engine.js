...
const { CATEGORY_FALLBACK_IMAGES } = require('./normalizer');
...
async function executeHybridFeed(options = {}) {
  ...
  const finalEvents = ranked.map(e => {
    ...
    return {
      ...
      image: e.canonical_image_url || e.image || CATEGORY_FALLBACK_IMAGES[e.category_tags?.[0]] || CATEGORY_FALLBACK_IMAGES.other,
      distance_miles: d,
      distanceMiles: d,
