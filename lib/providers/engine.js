...
const { resolveSourceQualityLabel, CATEGORY_FALLBACK_IMAGES, getCategoryFallbackImage } = require('./normalizer');
...
const { getActiveCommunityPosts, syncCommunityPostsFromDurableStore } = require('../community-posts/community-posts');
...
async function executeHybridFeed(options = {}) {
  ...
  if (Number.isFinite(lat) && Number.isFinite(lon)) {
    ...
    // 3. Autonomous Community Posts (instant public submissions scoped by broadcast radius)
    const shouldIncludeCommunity = options.enableCommunityPosts === true || (options.enableCommunityPosts !== false && fetchFn === fetch);
    if (shouldIncludeCommunity) {
      try {
        await syncCommunityPostsFromDurableStore();
      } catch (_) {}
      const activeCommunityPosts = getActiveCommunityPosts({
        lat,
        lon,
        radiusMiles,
        windowStart,
      ...
      if (activeCommunityPosts.length > 0) {
        providerResults.push(...activeCommunityPosts);
        communityCount += activeCommunityPosts.length;
      }
    }
  }
  ...
  const filtered = combined.filter(e => {
    ...
    // Filter by sourceFilter ('all', 'official' / 'indexed', 'community')
    const sourceFilter = String(options.sourceFilter || '').toLowerCase();
    const isCommunity = Boolean(e.source === 'community_post' || e.sourceType === 'community_submission' || e.isCommunityPost);
    if ((sourceFilter === 'official' || sourceFilter === 'indexed') && isCommunity) {
      return false;
    }
    if ((sourceFilter === 'community' || sourceFilter === 'user') && !isCommunity) {
      return false;
    }
  ...
  // 4. Distance filter (expanded for racing because tracks are located in exurbs, and clamped to event broadcastRadiusMiles)
  ...
  const withinRadius = withDistance.filter(e => {
    const maxAllowed = e.broadcastRadiusMiles ? Math.min(effectiveMaxRadius, e.broadcastRadiusMiles) : effectiveMaxRadius;
    if (e.distance_miles != null && e.distance_miles > maxAllowed) return false;
    return true;
  });
  ...
  const finalEvents = ranked.map(e => {
    ...
    const isEndingSoon = Boolean(e.end_time && (new Date(e.end_time).getTime() - Date.now()) <= 24 * 3600e3 && (new Date(e.end_time).getTime() - Date.now()) > 0);
      ...
      const rawSourceImage = e.canonical_image_url || e.image || null;
      const isOriginalImage = Boolean(rawSourceImage && !rawSourceImage.includes('unsplash.com'));
      const resolvedImage = rawSourceImage || getCategoryFallbackImage(e.category || e.category_tags?.[0], e.id || e.title) || CATEGORY_FALLBACK_IMAGES.other;
      ...
      return {
        ...
        image: resolvedImage,
        canonical_image_url: rawSourceImage || resolvedImage,
        isOriginalImage,
        distance_miles: d,
        distanceMiles: d,
