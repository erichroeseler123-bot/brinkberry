...
function normalizeCategory(rawCategory, tags = []) {
  ...
  // 1. Civic & Politics: First-class neutral public information category
  if (/city council|county board|school board|planning (and|&)? zoning|zoning|public hearing|state legislature|legislative|town hall|candidate forum|election|voter registration|budget meeting|advisory board|referendum|caucus|civic|politics|government|municipal|ordinance|commission/i.test(text)) return 'civic';
  // 2. Library, Maker & Community Gatherings
  if (/library|book club|storytime|workshop|makerspace|community center|neighborhood|volunteer|trivia|arcade|bowling|meetup/i.test(text)) return 'community';
  ...
  if (/sport|game|nba|nfl|mlb|nhl|soccer|match|tournament/i.test(text)) return 'sports';
  if (/kid|family|children|youth|parent/i.test(text)) return 'family';
  if (/food|drink|beer|wine|dining|tasting|brewery|cocktail|farmers market/i.test(text)) return 'food';
