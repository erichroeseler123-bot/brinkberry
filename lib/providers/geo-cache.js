...
class GeoCache {
  ...
  getCacheKey(provider, lat, lon, radius, window, mode) {
    const tile = this.getTile(lat, lon);
    const timeBucket = this.getTimeBucket(10);
    return `${provider}:${tile}:${radius}:${window}:${mode || 'all'}:${timeBucket}`;
  }

  get(provider, lat, lon, radius, window, mode) {
    const key = this.getCacheKey(provider, lat, lon, radius, window, mode);
  ...
  set(provider, lat, lon, radius, window, mode, data) {
    const key = this.getCacheKey(provider, lat, lon, radius, window, mode);
